...
echo ""
echo -e "${CYAN}6. Run the Build & Push Script:${NC}"
echo -e "   Run: ${GREEN}./build-and-push-acr.sh${NC}"
echo ""
echo -e "${CYAN}7. Run the set up script:${NC}"
echo -e "   Run: ${GREEN}./setup_app_config.sh${NC}"
echo ""
echo -e "${CYAN}8. Update Kubernetes manifests with correct ACR name:${NC}"
echo "   After infrastructure is deployed, get the ACR name from Azure and update"
echo "   the Docker image references in all k8s-manifest/*.yml files to use the"
echo "   correct ACR registry name (includes random suffix)."
echo ""
echo -e "${CYAN}9. Update ConfigMap with current infrastructure values:${NC}"
echo "   After deploying to Kubernetes, update the env-config ConfigMap with"
echo "   current connection strings and Key Vault URI from Terraform outputs."
echo "   This ensures pods can connect to the correct infrastructure resources."
echo ""
echo -e "${CYAN}10. Install NGINX Ingress Controller:${NC}"
echo "   Required for API routing and SSL certificates to work properly."
echo -e "   Run: ${GREEN}kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.8.2/deploy/static/provider/cloud/deploy.yaml${NC}"
echo ""
echo -e "${CYAN}11. Deploy to Kubernetes:${NC}"
echo -e "   Run: ${GREEN}kubectl apply -f k8s-manifest/${NC}"
echo ""
echo -e "${CYAN}12. Update DNS (Important):${NC}"
echo "   After ingress controller is ready, get the LoadBalancer IP and update"
echo "   your Namecheap DNS A record to point to the new external IP."
echo -e "   Check IP with: ${GREEN}kubectl get service -n ingress-nginx${NC}"
echo ""
