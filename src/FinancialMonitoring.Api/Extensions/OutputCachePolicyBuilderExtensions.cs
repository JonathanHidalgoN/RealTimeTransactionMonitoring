...
/// <summary>
/// Extension methods for IOutputCachePolicyBuilder to reduce configuration boilerplate
/// </summary>
public static class OutputCachePolicyBuilderExtensions
{
    /// <summary>
    /// Conditionally applies caching based on duration in seconds.
    /// If seconds > 0, applies the specified duration and optional configuration.
    /// If seconds <= 0, disables caching with NoCache().
    /// </summary>
    ...
    public static IOutputCachePolicyBuilder ConditionalExpire(
        this IOutputCachePolicyBuilder builder, 
        int seconds,
        Action<IOutputCachePolicyBuilder>? configurePolicy = null)
    {
        if (seconds > 0)
        {
